...
this.navigationMenusCache$ = this.http.get<NavigationMenu[]>(this.navigationMenusAPI).pipe(
...
this.showLoading$ = combineLatest([this.loading$, this.navigationService.navigationMenus$.pipe(startWith([]))]).pipe(map(([isLoading, cachedMenus]) => isLoading && cachedMenus.length === 0));
```
